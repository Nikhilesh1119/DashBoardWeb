...
function StudentPage () {
  ...
  return (
    <div
      className={`${isDarkMode ? 'bg-[#0d192f]' : 'bg-white'} min-h-screen flex justify-center  py-4 px-3`}
    >
      <div
        className={`${isDarkMode ? 'bg-[#152f54] bg-opacity-40' : 'bg-[#b9d7f1] bg-opacity-30'} flex flex-col max-w-full mx-auto p-6 w-[90%]`}
      >
        <div className="mb-4 flex">
          <div className="border border-red-500 inline-block bg-white">
            <button
              className={`${isDarkMode ? 'bg-[#152f54]' : 'bg-white'} text-red-500 font-bold p-2 `}
            >
              <Link
                to="/register-student"
        ...
        <div
          className={`${isDarkMode ? 'bg-[#0d192f]' : 'bg-white'}  shadow-md rounded-lg p-5`}
        >
          <Studentlist />
